 ...
 * @method     StandQuery orderByTarifa($order = Criteria::ASC) Order by the tarifa column
 *
 ...
 * @method     StandQuery groupByTarifa() Group by the tarifa column
 *
 ...
 * @method     Stand findOneByTarifa(string $tarifa) Return the first Stand filtered by the tarifa column
 *
 ...
 * @method     array findByTarifa(string $tarifa) Return Stand objects filtered by the tarifa column
 *
...
abstract class BaseStandQuery extends ModelCriteria
{
	...
	/**
	 * Filter the query on the tarifa column
	 * 
	 * Example usage:
	 * <code>
	 * $query->filterByTarifa('fooValue');   // WHERE tarifa = 'fooValue'
	 * $query->filterByTarifa('%fooValue%'); // WHERE tarifa LIKE '%fooValue%'
	 * </code>
	 *
	 * @param     string $tarifa The value to use as filter.
	 *              Accepts wildcards (* and % trigger a LIKE)
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    StandQuery The current query, for fluid interface
	 */
	public function filterByTarifa($tarifa = null, $comparison = null)
	{
		if (null === $comparison) {
			if (is_array($tarifa)) {
				$comparison = Criteria::IN;
			} elseif (preg_match('/[\%\*]/', $tarifa)) {
				$tarifa = str_replace('*', '%', $tarifa);
				$comparison = Criteria::LIKE;
			}
		}
		return $this->addUsingAlias(StandPeer::TARIFA, $tarifa, $comparison);
	}
