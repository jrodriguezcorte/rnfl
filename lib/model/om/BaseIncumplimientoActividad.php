...
/**
 * Base class that represents a row from the 'incumplimiento_actividad' table.
 *
 * 
 *
 ...
 */
abstract class BaseIncumplimientoActividad extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'IncumplimientoActividadPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        IncumplimientoActividadPeer
	 */
	 ...
	 * @return     IncumplimientoActividad The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = IncumplimientoActividadPeer::ID;
		}
	...
	public function setNombre($v)
	{
		...
		if ($this->nombre !== $v) {
			$this->nombre = $v;
			$this->modifiedColumns[] = IncumplimientoActividadPeer::NOMBRE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = IncumplimientoActividadPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating IncumplimientoActividad object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = IncumplimientoActividadPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseIncumplimientoActividad:delete:pre') as $callable)
			{
			...
			if ($ret) {
				IncumplimientoActividadQuery::create()
					->filterByPrimaryKey($this->getPrimaryKey())
					->delete($con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseIncumplimientoActividad:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseIncumplimientoActividad:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseIncumplimientoActividad:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				IncumplimientoActividadPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(IncumplimientoActividadPeer::ID) ) {
					...
					$affectedRows = IncumplimientoActividadPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = IncumplimientoActividadPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = IncumplimientoActividadPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['IncumplimientoActividad'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['IncumplimientoActividad'][$this->getPrimaryKey()] = true;
		$keys = IncumplimientoActividadPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(IncumplimientoActividadPeer::ID)) $criteria->add(IncumplimientoActividadPeer::ID, $this->id);
		if ($this->isColumnModified(IncumplimientoActividadPeer::NOMBRE)) $criteria->add(IncumplimientoActividadPeer::NOMBRE, $this->nombre);
	...
	{
		$criteria = new Criteria(IncumplimientoActividadPeer::DATABASE_NAME);
		$criteria->add(IncumplimientoActividadPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of IncumplimientoActividad (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     IncumplimientoActividad Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     IncumplimientoActividadPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new IncumplimientoActividadPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this IncumplimientoActividad is new, it will return
		...
		if(null === $this->collIncumplmientoActividadFinalizadas || null !== $criteria) {
			if ($this->isNew() && null === $this->collIncumplmientoActividadFinalizadas) {
				...
				$collIncumplmientoActividadFinalizadas = IncumplmientoActividadFinalizadaQuery::create(null, $criteria)
					->filterByIncumplimientoActividad($this)
					->find($con);
				if (null !== $criteria) {
	...
	public function addIncumplmientoActividadFinalizada(IncumplmientoActividadFinalizada $l)
	{
		...
		if (!$this->collIncumplmientoActividadFinalizadas->contains($l)) { // only add it if the **same** object is not already associated
			...
			$l->setIncumplimientoActividad($this);
		}
	}
	 ...
	 * Otherwise if this IncumplimientoActividad is new, it will return
	 * an empty collection; or if this IncumplimientoActividad has previously
	 * been saved, it will retrieve related IncumplmientoActividadFinalizadas from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in IncumplimientoActividad.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(IncumplimientoActividadPeer::DEFAULT_STRING_FORMAT);
	}
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseIncumplimientoActividad:' . $name))
		{
