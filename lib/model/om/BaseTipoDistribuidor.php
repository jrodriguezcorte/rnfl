...
/**
 * Base class that represents a row from the 'tipo_distribuidor' table.
 *
 * 
 *
 ...
 */
abstract class BaseTipoDistribuidor extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'TipoDistribuidorPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        TipoDistribuidorPeer
	 */
	 ...
	 * @return     TipoDistribuidor The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = TipoDistribuidorPeer::ID;
		}
	...
	public function setNombre($v)
	{
		...
		if ($this->nombre !== $v) {
			$this->nombre = $v;
			$this->modifiedColumns[] = TipoDistribuidorPeer::NOMBRE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = TipoDistribuidorPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating TipoDistribuidor object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = TipoDistribuidorPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseTipoDistribuidor:delete:pre') as $callable)
			{
			...
			if ($ret) {
				TipoDistribuidorQuery::create()
					->filterByPrimaryKey($this->getPrimaryKey())
					->delete($con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTipoDistribuidor:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseTipoDistribuidor:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTipoDistribuidor:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				TipoDistribuidorPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(TipoDistribuidorPeer::ID) ) {
					...
					$affectedRows = TipoDistribuidorPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = TipoDistribuidorPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = TipoDistribuidorPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['TipoDistribuidor'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['TipoDistribuidor'][$this->getPrimaryKey()] = true;
		$keys = TipoDistribuidorPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(TipoDistribuidorPeer::ID)) $criteria->add(TipoDistribuidorPeer::ID, $this->id);
		if ($this->isColumnModified(TipoDistribuidorPeer::NOMBRE)) $criteria->add(TipoDistribuidorPeer::NOMBRE, $this->nombre);
	...
	{
		$criteria = new Criteria(TipoDistribuidorPeer::DATABASE_NAME);
		$criteria->add(TipoDistribuidorPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of TipoDistribuidor (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     TipoDistribuidor Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     TipoDistribuidorPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new TipoDistribuidorPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this TipoDistribuidor is new, it will return
	 ...
	 * Otherwise if this TipoDistribuidor is new, it will return
	 * an empty collection; or if this TipoDistribuidor has previously
	 * been saved, it will retrieve related ExpositorFerias from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in TipoDistribuidor.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(TipoDistribuidorPeer::DEFAULT_STRING_FORMAT);
	}
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseTipoDistribuidor:' . $name))
		{
