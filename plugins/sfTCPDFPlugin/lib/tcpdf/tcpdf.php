...
class TCPDF {
	...
	public function Footer() {
		...
		if (empty($this->pagegroups)) {
			$anio = date('Y'); 
			$pagenumtxt = 'FEPR / '.$anio;
			$valor = $this->getAliasNumPage()." / ".$this->getAliasNbPages();
		} else {
		...
		if ($this->getRTL()) {
			$this->SetX($this->original_lMargin);
                        ...
                        $this->MultiCell(20, 5, $pagenumtxt, 0, 'J', 0, 0);
                        $this->MultiCell(150, 5, "", 0, 'J', 0, 0);
                        $this->MultiCell(15, 5, $valor, 0, 'J', 0);
		} else {
			$this->SetX($this->original_lMargin);
                        $this->Cell(0, 0, '', 'T', 1, 'R');
                        $this->SetFont("times", "B", 8);
